...
try {
    ...
    if (fs.existsSync(adminFile)) {
        ...
        const existingAdmin = adminDb.getByUsername(adminData.username);
        if (!existingAdmin) {
            adminDb.create(adminData.username, adminData.password);
    ...
    if (fs.existsSync(productsFile)) {
        ...
        products.forEach(product => {
            try {
                ...
                const existing = productDb.getById(product.id);
                if (!existing) {
                    // Insert with specific ID if provided
                    productDb.create({
                        name: product.name,
        ...
        if (subscribers.length > 0) {
            ...
            subscribers.forEach(subscriber => {
                try {
                    ...
                    const existing = subscriberDb.getByEmail(email);
                    if (!existing) {
